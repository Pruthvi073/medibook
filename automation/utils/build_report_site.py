"""
build_report_site.py — Staging reports for GitHub Pages.
"""
import argparse
import json
import os
...
<body>
  <h1>MediBook — Mobile E2E Test Reports</h1>
  <p class="subtitle">Appium Mobile E2E · GitHub Pages Portal</p>
  ...
  <footer>MediBook Appium Mobile E2E Framework · DevOps CI/CD</footer>
</body>
</html>"""
...
def build(run_number: str, base_url: str, report_src: str, out_dir: str,
          json_report: str = "execution-results.json"):
    src      = Path(report_src)
    ...
    for b in reversed(list(builds)):
        ...
        card = BUILD_CARD_TEMPLATE
        card = card.replace("{run_number}", bnum)
        card = card.replace("{date}", now if b.name == f"build-{run_number.zfill(3)}" else "previous run")
        card = card.replace("{passed}", str(bs["passed"]))
        card = card.replace("{failed}", str(bs["failed"]))
        card = card.replace("{total}", str(bs["total"]))
        card = card.replace("{pct}", str(bs["pct"]))
        card = card.replace("{report_url}", burl)
        history_html_parts.append(card)

    portal_html = PORTAL_TEMPLATE
    portal_html = portal_html.replace("{base_url}", base_url.rstrip("/") + "/")
    portal_html = portal_html.replace("{generated_at}", now)
    portal_html = portal_html.replace("{history_html}", "\n".join(history_html_parts) or "<p style='color:#64748b'>No builds yet.</p>")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--report-src",  default="automation/Test Results")
    parser.add_argument("--out-dir",     default="./gh-pages-reports")
    parser.add_argument("--json",        default="automation/reports/execution-results.json")
